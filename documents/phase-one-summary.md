...
- **Genius 详情流程**：新增 `src/lib/song-details.ts`，提供 `ensureSongDetails` 及 `ensureSongDetailsByGeniusId` 两个入口，优先命中数据库缓存，未命中时再调用 `fetchGeniusSongDetails`，并在入库前将 `description.dom` 及注释 `body.dom` 转换为 Markdown 字符串覆盖写入。
- **页面整合**：在歌曲详情页加载流程中接入 `ensureSongDetails`，首次访问时自动抓取并入库，但暂不在界面渲染详情字段。

## 后续建议
1. 挑选并格式化 `details` 中需要展示的字段，规划 UI 呈现。
2. 视需求增加失效策略（例如定期刷新或手动触发详情更新）。
3. 选用 Markdown 渲染组件规划页面展示布局，挑选需要呈现的字段。
